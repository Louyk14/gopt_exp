...
substrait::Rel* PhysicalHashAggregate::ToSubstraitClass(unordered_map<int, string>& tableid2name) const {
    ...
    for (int i = 0; i < grouped_aggregate_data.groups.size(); ++i) {
        BoundReferenceExpression *expr = (BoundReferenceExpression *) grouped_aggregate_data.groups[i].get();
        emit->add_output_mapping(expr->index);
        emit->add_output_types(TypeIdToString(expr->return_type.InternalType()));
        emit->add_output_exp_type(ExpressionTypeToString(expr->type));
    }

    aggregate->set_type(PhysicalOperatorToString(type));

    for (int i = 0; i < grouped_aggregate_data.aggregates.size(); ++i) {
        substrait::AggregateRel_Measure* measure = new substrait::AggregateRel_Measure();
        substrait::AggregateFunction* aggregate_func = new substrait::AggregateFunction();

        BoundAggregateExpression* expr = (BoundAggregateExpression*) grouped_aggregate_data.aggregates[i].get();
        substrait::Type* output_type = new substrait::Type();

        if (expr->return_type.InternalType() == PhysicalType::INT32) {
            substrait::Type_I32* type_i32 = new substrait::Type_I32();
            output_type->set_allocated_i32(type_i32);
        }
        else if (expr->return_type.InternalType() == PhysicalType::VARCHAR) {
            substrait::Type_String* type_str = new substrait::Type_String();
            output_type->set_allocated_string(type_str);
        }
        aggregate_func->set_allocated_output_type(output_type);

        if (expr->function.name == "min") {
            aggregate_func->set_function_reference(1);
        }
        else if (expr->function.name == "max") {
            aggregate_func->set_function_reference(2);
        }
        else if (expr->function.name == "avg") {
            aggregate_func->set_function_reference(3);
        }
        else if (expr->function.name == "string_agg") {
            aggregate_func->set_function_reference(4);
        }

        for (int j = 0; j < expr->function.arguments.size(); ++j) {
            substrait::FunctionArgument* func_arg = new substrait::FunctionArgument();
            string* type_str = new string(TypeIdToString(expr->function.arguments[j].InternalType()));
            func_arg->set_allocated_enum_(type_str);
            *aggregate_func->add_arguments() = *func_arg;
            delete func_arg;
        }

        for (int j = 0; j < expr->children.size(); ++j) {
            substrait::FunctionArgument* func_arg = new substrait::FunctionArgument();
            BoundReferenceExpression* child = (BoundReferenceExpression*) expr->children[j].get();
            string* index_str = new string(to_string(child->index));
            func_arg->set_allocated_enum_(index_str);
            *aggregate_func->add_childs() = *func_arg;
            delete func_arg;
        }

        //StringAggBindData* bind = (StringAggBindData*) expr->bind_info.get();
        //aggregate_func->set_binder(bind->sep);

        measure->set_allocated_measure(aggregate_func);
