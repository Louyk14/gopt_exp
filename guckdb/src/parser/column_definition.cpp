...
ColumnDefinition::ColumnDefinition(string name_p, LogicalType type_p)
    : name(std::move(name_p)), type(std::move(type_p)), rai_oid(0) {
}

ColumnDefinition::ColumnDefinition(string name_p, LogicalType type_p, unique_ptr<ParsedExpression> expression,
                                   TableColumnType category)
    : name(std::move(name_p)), type(std::move(type_p)), category(category), expression(std::move(expression)), rai_oid(0) {
}
...
void ColumnDefinition::SetRAIOid(idx_t oid) {
    this->rai_oid = oid;
}
...
idx_t ColumnDefinition::GetRAIOid() const {
    return rai_oid;
}
