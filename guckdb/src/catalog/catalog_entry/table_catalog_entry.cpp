...
TableCatalogEntry::TableCatalogEntry(Catalog &catalog, SchemaCatalogEntry &schema, CreateTableInfo &info)
    : StandardEntry(CatalogType::TABLE_ENTRY, schema, catalog, info.table), columns(std::move(info.columns)),
      constraints(std::move(info.constraints)), rai_column_index(0) {
...
void TableCatalogEntry::AddRAIColumns(ClientContext& context, vector<column_t> column_ids, vector<column_t> &updated_columns) {
    column_t column_id = columns.PhysicalColumnCount();
    if (rai_column_index == 0) {
        rai_column_index = column_id;
    }
    for (idx_t i = 0; i < 2; i++) {
        auto col_id = column_ids[i];
        if (columns.GetColumn(LogicalIndex(col_id)).GetRAIOid() == 0) {
            string col_name = columns.GetColumn(LogicalIndex(col_id)).Name() + "_rowid";
            ColumnDefinition rai_col(col_name, LogicalType::BIGINT);
            rai_col.SetOid(column_id++);
            columns.AddColumn(rai_col.Copy());
            columns.GetColumnMutable(LogicalIndex(col_id)).SetRAIOid(rai_col.Oid());
            updated_columns[i] = rai_col.Oid();
            // ColumnDefinition* col_def = &columns.GetColumnMutable(LogicalIndex(col_id));
            // col_def->SetRAIOid(rai_col.Oid());
            this->GetStorage().AppendRAIColumn(context, rai_col, rai_col.Oid());
        }
    }
}
