	...
	for (auto &entry : equivalence_map) {
		...
		for (idx_t i = 0; i < entries.size(); i++) {
			for (idx_t k = i + 1; k < entries.size(); k++) {
                // remove the constraints between two edges
                /*auto entryi = (BoundColumnRefExpression*) entries[i].get();
                auto entryk = (BoundColumnRefExpression*) entries[k].get();
                if (entries.size() > 2 && entryi->binding.table && entryk->binding.table &&
                    !entryi->binding.table->storage->info->rais.empty() &&
                    !entryk->binding.table->storage->info->rais.empty()) {
                    continue;
                }*/
