...
unique_ptr<RowGroup> RowGroup::AddColumn(RowGroupCollection &new_collection, int &column_index, LogicalType& new_type, Vector &result, int rows_to_write) {
    Verify();

    // construct a new column data for the new column
    auto added_column =
            ColumnData::CreateColumn(GetBlockManager(), GetTableInfo(), GetColumnCount(), start, new_type);
