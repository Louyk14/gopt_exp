...
shared_ptr<RowGroupCollection> RowGroupCollection::CreateAppendColumnRAI(int& column_index, LogicalType type, duckdb::Vector &vector, int chunk_count) {
    ...
    RowGroup* row_group = row_groups->GetRootSegment();

    // auto &changed_stats = result->stats.GetStats(column_index);

    result->stats.InitializeAddColumn(stats, type);
    auto &new_column_stats = result->stats.GetStats(column_index);
    int left_count = chunk_count;

    while (row_group) {
        idx_t group_size = MinValue<idx_t>(left_count, row_group->GetColumn(0).count);
        new_vector.Slice(vector, offset, offset + group_size);
        auto new_row_group = row_group->AddColumn(*result, column_index, type, new_vector, group_size);
        new_row_group->MergeIntoStatistics(column_index, new_column_stats.Statistics());
        result->row_groups->AppendSegment(std::move(new_row_group));
        offset += group_size;
        left_count -= group_size;
...
void RowGroupCollection::AppendColumnRAI(int column_idx, ColumnAppendState& state, duckdb::Vector &vector, int chunk_count) {
    idx_t current_total_rows = 0;
    int offset = 0;
    Vector new_vector(vector);
    RowGroup* row_group = row_groups->GetRootSegment();
    while (row_group) {
        idx_t group_size = row_group->GetColumn(column_idx).count;
        new_vector.Slice(vector, offset, offset + group_size);
        row_group->GetColumn(column_idx).Append(state, new_vector, group_size);
        offset += group_size;
        row_group = row_groups->GetNextSegment(row_group);
    }
}
