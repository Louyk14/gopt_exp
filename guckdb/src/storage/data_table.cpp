...
void DataTable::VerifyAppendConstraints(TableCatalogEntry &table, ClientContext &context, DataChunk &chunk,
	...
	for (idx_t i = 0; i < bound_constraints.size(); i++) {
		...
		case ConstraintType::NOT_NULL: {
			...
			auto &col = table.GetColumns().GetColumn(not_null.index);
...
void DataTable::AppendRAIColumn(ClientContext &context, ColumnDefinition& col_def, column_t oid) {
    column_t columns_count = this->GetTypes().size();
    assert(columns_count == oid);
    // col_def.SetOid(oid);
    col_def.SetStorageOid(oid);
    column_definitions.emplace_back(col_def.Copy());
    // BoundConstantExpression expr(Value((int64_t)0));
    // row_groups = row_groups->AddColumn(context, col_def, expr);
}
