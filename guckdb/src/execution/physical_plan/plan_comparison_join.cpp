...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::PlanComparisonJoin(LogicalComparisonJoin &op) {
    ...
    D_ASSERT((op.op_mark == OpMark::MERGED_SIP_JOIN || op.op_mark == OpMark::ADAPTIVE_MERGE_SIP_JOIN)
           ? op.children.size() == 3
           : op.children.size() == 2);
    ...
    if (op.op_mark == OpMark::SIP_JOIN) {
        vector<LogicalType> delim_types;
        return make_uniq<PhysicalSIPJoin>(op, move(left), move(right), move(op.conditions), op.join_type,
                                            op.left_projection_map, op.right_projection_map, delim_types, op.estimated_cardinality);
    }
    else if (op.op_mark == OpMark::MERGED_SIP_JOIN) {
        vector<LogicalType> delim_types;
        return make_uniq<PhysicalMergeSIPJoin>(op, move(left), move(right), move(op.conditions), op.join_type,
                                            op.left_projection_map, op.right_projection_map, op.merge_projection_map, delim_types, op.estimated_cardinality);
...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::PlanSIPJoin(LogicalComparisonJoin &op) {
    // now visit the children
