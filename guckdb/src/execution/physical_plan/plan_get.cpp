...
unique_ptr<PhysicalOperator> PhysicalPlanGenerator::CreatePlan(LogicalGet &op) {
	...
	if (!op.function.projection_pushdown) {
        ...
        if (op.op_mark == OpMark::SCAN) {
            auto node = make_uniq<PhysicalLookup>(op.returned_types, op.function, op.table_index, std::move(op.bind_data),
		...
		auto node = make_uniq<PhysicalTableScan>(op.returned_types, op.function, op.table_index, std::move(op.bind_data),
		                                         op.returned_types, op.column_ids, std::move(op.expressions), vector<column_t>(), op.names,
		                                         std::move(table_filters), op.estimated_cardinality, op.extra_info);
		// first check if an additional projection is necessary
		if (op.column_ids.size() == op.returned_types.size()) {
			bool projection_necessary = false;
        ...
        if (op.op_mark == OpMark::SCAN) {
            return make_uniq<PhysicalLookup>(op.types, op.function, op.table_index, std::move(op.bind_data), op.returned_types,
                                                op.column_ids, op.projection_ids, op.names, std::move(table_filters),
                                                op.estimated_cardinality, op.extra_info);
        }
		return make_uniq<PhysicalTableScan>(op.types, op.function, op.table_index, std::move(op.bind_data), op.returned_types,
		                                    op.column_ids, move(op.expressions), op.projection_ids, op.names, std::move(table_filters),
		                                    op.estimated_cardinality, op.extra_info);
	}
}
