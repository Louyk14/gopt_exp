...
JoinHashTable::JoinHashTable(BufferManager &buffer_manager, vector<JoinCondition> &conditions, vector<TypeId> btypes,
    ...
    : buffer_manager(buffer_manager), build_types(move(btypes)), equality_size(0), condition_size(0), build_size(0),
      entry_size(0), tuple_size(0), join_type(join_type), finalized(false), has_null(false), count(0) {
...
void JoinHashTable::Initialize() {
    finalized = false;

    if (join_type == JoinType::MARK && !correlated_mark_join_info.correlated_types.empty()) {
        correlated_mark_join_info.group_chunk.Reset();
        correlated_mark_join_info.payload_chunk.Reset();
    }
...
unique_ptr<ScanStructure> JoinHashTable::Probe(DataChunk &keys) {
	...
	auto probe_hash_start = std::chrono::high_resolution_clock::now();
#endif
	Vector hashes(TypeId::HASH);
...
#ifdef ENABLE_PROFILING
	auto probe_hash_end = std::chrono::high_resolution_clock::now();
#endif
	...
	ss->count = count;
#ifdef ENABLE_PROFILING
	probe_hash_time += (probe_hash_end - probe_hash_start).count();
#endif
	return ss;
}
