...
namespace duckdb {
    ...
    using SIPValidityBytes = SIPHashTable::ValidityBytes;
    using SIPScanStructure = SIPHashTable::SIPScanStructure;
    using SIPProbeSpill = SIPHashTable::SIPProbeSpill;
    using SIPProbeSpillLocalState = SIPHashTable::SIPProbeSpillLocalAppendState;
    ...
    void SIPHashTable::Merge(SIPHashTable &other) {
        {
    ...
    void SIPHashTable::ApplyBitmask(Vector &hashes, idx_t count) {
    ...
    void SIPHashTable::ApplyKeymask(Vector &hashes, const SelectionVector &sel, idx_t count, Vector &pointers) {
        UnifiedVectorFormat hdata;
        hashes.ToUnifiedFormat(count, hdata);
        ...
        for (idx_t i = 0; i < count; i++) {
            ...
            auto hindex = hdata.sel->get_index(rindex);
            auto hash = hash_data[hindex];
            result_data[rindex] = main_ht + hash;
        }
    }

    void SIPHashTable::Hash(DataChunk &keys, const SelectionVector &sel, idx_t count, Vector &hashes) {
        if (count == keys.size()) {
    ...
    static idx_t SIPFilterNullValues(UnifiedVectorFormat &vdata, const SelectionVector &sel, idx_t count,
    ...
    idx_t SIPHashTable::PrepareKeys(DataChunk &keys, unsafe_unique_array<UnifiedVectorFormat> &key_data,
    ...
    void SIPHashTable::Build(PartitionedTupleDataAppendState &append_state, DataChunk &keys, DataChunk &payload) {
        D_ASSERT(!finalized);
    ...
    static inline void SIPInsertHashesLoop(atomic<data_ptr_t> pointers[], const hash_t indices[], const idx_t count,
    ...
    void SIPHashTable::InsertHashes(Vector &hashes, idx_t count, data_ptr_t key_locations[], bool parallel) {
        ...
        if (parallel) {
            SIPInsertHashesLoop<true>(pointers, indices, count, key_locations, pointer_offset);
        } else {
            SIPInsertHashesLoop<false>(pointers, indices, count, key_locations, pointer_offset);
        }
    }

    void SIPHashTable::InitializePointerTable() {
    ...
    void SIPHashTable::Finalize(idx_t chunk_idx_from, idx_t chunk_idx_to, bool parallel) {
    ...
    unique_ptr<SIPScanStructure> SIPHashTable::InitializeScanStructure(DataChunk &keys, const SelectionVector *&current_sel) {
        D_ASSERT(Count() > 0); // should be handled before
        D_ASSERT(finalized);

        // set up the scan structure
        auto ss = make_uniq<SIPScanStructure>(*this);
    ...
    unique_ptr<SIPScanStructure> SIPHashTable::Probe(DataChunk &keys, Vector *precomputed_hashes) {
        const SelectionVector *current_sel;
    ...
    void SIPScanStructure::InitializeSelectionVector(const SelectionVector *&current_sel) {
    ...
    void SIPScanStructure::GatherResult(Vector &result, const SelectionVector &sel_vector, const idx_t count,
                                     const idx_t col_idx) {
    ...
    template <bool MATCH>
    void SIPScanStructure::NextSemiOrAntiJoin(DataChunk &keys, DataChunk &left, DataChunk &result) {
    ...
    void SIPHashTable::ScanFullOuter(SIPHTScanState &state, Vector &addresses, DataChunk &result) {
    ...
    idx_t SIPHashTable::FillWithHTOffsets(SIPHTScanState &state, Vector &addresses) {
        // iterate over HT
        auto key_locations = FlatVector::GetData<data_ptr_t>(addresses);
        idx_t key_count = 0;
    ...
    bool SIPHashTable::RequiresExternalJoin(ClientConfig &config, vector<unique_ptr<SIPHashTable>> &local_hts) {
        total_count = 0;
        idx_t data_size = 0;
    ...
    void SIPHashTable::Unpartition() {
        for (auto &partition : sink_collection->GetPartitions()) {
            data_collection->Combine(*partition);
        }
    }

    bool SIPHashTable::RequiresPartitioning(ClientConfig &config, vector<unique_ptr<SIPHashTable>> &local_hts) {
    ...
    void SIPHashTable::Partition(SIPHashTable &global_ht) {
    ...
    void SIPHashTable::Reset() {
        data_collection->Reset();
        finalized = false;
    }

    bool SIPHashTable::PrepareExternalFinalize() {
        if (finalized) {
            Reset();
        }
    ...
    static void SIPCreateSpillChunk(DataChunk &spill_chunk, DataChunk &keys, DataChunk &payload, Vector &hashes) {
        spill_chunk.Reset();
    ...
    unique_ptr<SIPScanStructure> SIPHashTable::ProbeAndSpill(DataChunk &keys, DataChunk &payload, SIPProbeSpill &probe_spill,
                                                             SIPProbeSpillLocalAppendState &spill_state,
        ...
        SIPCreateSpillChunk(spill_chunk, keys, payload, hashes);
    ...
    SIPProbeSpill::SIPProbeSpill(SIPHashTable &ht, ClientContext &context, const vector<LogicalType> &probe_types)
    ...
    SIPProbeSpillLocalState SIPProbeSpill::RegisterThread() {
        SIPProbeSpillLocalAppendState result;
    ...
    void SIPProbeSpill::Append(DataChunk &chunk, SIPProbeSpillLocalAppendState &local_state) {
        if (partitioned) {
    ...
    void SIPProbeSpill::PrepareNextProbe() {
        if (partitioned) {
