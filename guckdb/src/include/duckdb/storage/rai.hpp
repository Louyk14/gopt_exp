...
struct RAIInfo {
	...
	string RAITypeToString() {
		switch (rai_type) {
		case RAIType::INVALID:
			return "INVALID";
		...
		case RAIType::SELF:
			return "SELF";
		default:
			throw ConversionException("Invalid RAIType %d", rai_type);
		}
	}

	RAIType StringToRAIType(string type) {
		if (type == "INVALID")
			return RAIType::INVALID;
		else if (type == "TARGET_EDGE")
			return RAIType::TARGET_EDGE;
		else if (type == "SOURCE_EDGE")
			return RAIType::SOURCE_EDGE;
		else if (type == "EDGE_TARGET")
			return RAIType::EDGE_TARGET;
		else if (type == "EDGE_SOURCE")
			return RAIType::EDGE_SOURCE;
		else if (type == "SELF")
			return RAIType::SELF;
		else
			throw ConversionException("Invalid RAIType " + type);
	}
