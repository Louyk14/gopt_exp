...
public:
    ...
    shared_ptr<RowGroupCollection> CreateAppendColumnRAI(int& column_index, LogicalType type, duckdb::Vector &vector, int chunk_count);
    void AppendColumnRAI(int column_idx, ColumnAppendState& state, duckdb::Vector &vector, int chunk_count);
