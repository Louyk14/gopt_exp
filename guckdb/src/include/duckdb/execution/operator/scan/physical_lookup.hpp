...
//                         DuckDB
//
// duckdb/execution/operator/scan/physical_lookup.hpp
//
//
//===----------------------------------------------------------------------===//
...
//! Represents a scan of a base table
    ...
    public:
        static constexpr const PhysicalOperatorType TYPE = PhysicalOperatorType::LOOKUP;

    public:
        //! Table scan that immediately projects out filter columns that are unused in the remainder of the query plan
        PhysicalLookup(vector<LogicalType> types, TableFunction function, idx_t table_index, unique_ptr<FunctionData> bind_data,
    ...
    public:
        ...
        template <class T>
        void inline Lookup(ClientContext &context, shared_ptr<RowGroupSegmentTree>& row_groups, row_t *row_ids, Vector &result, idx_t count,
                          idx_t col_index, idx_t type_size = sizeof(T));
