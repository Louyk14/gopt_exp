...
void getStringListFromFile(string filename, int index, int count, std::vector<string>& slist) {
...
/*std::unique_ptr<PhysicalTableScan> get_scan_function(ClientContext& context, string& table_name, idx_t table_index,
                                                std::vector<idx_t>& columns_ids, std::vector<PhysicalType>& getTypes,
                                                unordered_map<idx_t, std::vector<TableFilter>> table_filters = unordered_map<idx_t, std::vector<TableFilter>>()) {


    auto& table_or_view =
            Catalog::GetCatalog(context, "").GetEntry(context, CatalogType::TABLE_ENTRY, "", table_name);

    idx_t table_index, TableFunction function, unique_ptr<FunctionData> bind_data,
            vector<LogicalType> returned_types, vector<string> returned_names;
    auto table = (TableCatalogEntry *)&table_or_view;
    auto logical_get = make_uniq<LogicalGet>(table_index, table->GetScanFunction(context, FunctionData()), );
