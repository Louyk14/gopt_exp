...
unique_ptr<JoinNode> JoinOrderOptimizer::CreateJoinTree(JoinRelationSet *set, NeighborInfo *info, JoinNode *left,
	...
	if (info->filters.size() == 0) {
		...
		if (cached_cardinalities.find(set) != cached_cardinalities.end()) {
			...
			expected_cardinality = std::max(left->cardinality, right->cardinality); //std::max(left->cardinality, right->cardinality);
		}
		expected_cost = left->cost + right->cost + std::max(left->cardinality, right->cardinality);//std::max(left->cardinality, right->cardinality);
	}
...
void JoinOrderOptimizer::findGetWithFilters(LogicalOperator* plan, vector<idx_t>& table_indices) {
	if (plan->type == LogicalOperatorType::GET) {
		LogicalGet* getPlan = (LogicalGet*) plan;
		if (!getPlan->tableFilters.empty()) {
			table_indices.push_back(getPlan->table_index);
		}
	}
	if (!plan->children.empty()) {
		for (int i = 0; i < plan->children.size(); ++i) {
			findGetWithFilters(plan->children[i].get(), table_indices);
		}
	}
}

void JoinOrderOptimizer::SetCardinality(vector<unordered_set<idx_t>>& join_sets, vector<idx_t>& cards) {
	for (int i = 0; i < cards.size(); ++i) {
		auto rel_set = set_manager.GetJoinRelation(join_sets[i]);
		cached_cardinalities[rel_set] = cards[i];
	}
}
...
unique_ptr<LogicalOperator> JoinOrderOptimizer::Optimize(unique_ptr<LogicalOperator> plan) {
	...
	for (int i = 0; i < table_indices.size(); ++i) {
		unordered_set<idx_t> join_set({ relation_mapping[table_indices[i]] });
